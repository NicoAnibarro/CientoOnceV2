...
export const Input = ({ icon, style, multiline, ...props }) => (
...
export const IconButton = ({ icon, onPress, active, danger }) => (
  ...
  >
    <Ionicons
      name={icon}
      size={20}
      color={danger ? colors.danger : active ? "#FFF" : colors.primaryDark}
    />
  </Pressable>
);
...
export const Empty = ({ text = "No hay registros para mostrar" }) => (
  <View style={styles.empty}>
    <Ionicons name="leaf-outline" size={34} color={colors.border} />
...
export function Pagination({ page, totalPages, onChange }) {
  ...
  const pages = Array.from(
    ...
    (number) =>
      Math.abs(number - page) <= 2 || number === 1 || number === totalPages,
  );
  return (
    <View style={styles.pagination}>
      ...
      {pages.map((number, index) => (
        <React.Fragment key={number}>
          {index && number - pages[index - 1] > 1 ? (
            <Text style={styles.dots}>…</Text>
          ) : null}
          <Pressable
            onPress={() => onChange(number)}
            style={[styles.page, page === number && styles.pageActive]}
          >
            <Text
              style={[styles.pageText, page === number && { color: "#FFF" }]}
            >
              {number}
            </Text>
          </Pressable>
...
const createStyles = () =>
  StyleSheet.create({
    ...
    compact: { minHeight: 40, paddingHorizontal: 13, borderRadius: 11 },
